...
This solution demonstrates the improvements that Web Assembly can offer over JavaScript, both when it comes to execution time and memory usage. While the results show that both of these solutions perform worse than native code, they have much more flexibility and so are still beneficial. These results are in line with the results found in the related work section of this paper, showing that the conclusions reached extend to image processing.

However the ease of programming in JavaScript is much better than in Rust due to many factors. One of these is that JavaScript has dynamic types, making it faster to code without having to think about what type a variable should be. JavaScript also has a much wider range of built-in functions which came in useful when creating the algorithm, such as the \texttt{map} function which condensed the size of the matrix multiplication code.
